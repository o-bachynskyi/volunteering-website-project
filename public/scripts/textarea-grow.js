document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('input', (e) => {
    if (e.target.matches('#add-post-text, #edit-post-text, #profile-text, #answer-text, #report-text')) {
      autoResize(e.target);
    }
  });
  ...
  document.addEventListener('click', (e) => {
    ...
    if (e.target.closest('.close-request-button')) {
      setTimeout(() => {
        const reportTextarea = document.getElementById('report-text');
        if (reportTextarea) autoResize(reportTextarea);
      }, 50);
    }
  });
});
